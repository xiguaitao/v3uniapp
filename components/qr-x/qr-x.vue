<template>
	<view class="canvasBox">
		<template v-if="isUse">
			...
			<view class="mask2 mask" :style="'width:' + maskWidth + 'px;top:' + maskHeight + 'px;height:' + 300 + 'px'">
			</view>
			<view class="mask3 mask" :style="'height:' + mask3Height + 'px;'"></view>
			<view class="mask4 mask" :style="'width:' + maskWidth + 'px;top:' + maskHeight + 'px;height:' + 300 + 'px'">
			</view>
		</template>
		<template v-else>
			<slot name="error">
...
export default {
	...
	computed: {
		mask3Height() {
			return this.windowHeight - (this.maskHeight + 300);
		}
	},
	methods: {

		getIOSVersion() {
			// 使用 uni.getSystemInfoSync() 方法获取系统信息
			const systemInfo = uni.getSystemInfoSync();
			// 如果是 iOS 系统，获取 iOS 版本号
			if (systemInfo.platform === 'ios') {
				console.log('systemInfo::: ', systemInfo);
				const version = systemInfo.system.match(/iPhone OS (\d+)/);
		...
		drawLine(begin, end, color = '#00ff33') {
		...
		createMsk() {
			...
			// this.maskHeight = (this.windowHeight / 2 - this.canvasHeight / 2) - 100
			// 获取.box元素相对于.canvasBox的顶部偏移量

			let that = this;
			const query = uni.createSelectorQuery();
			query.select('.box').boundingClientRect(); //.swiper是swiper类名，获取当前swiper距离顶部的位置
			query.exec(res => {
				console.log('swiper距离页面顶部的距离', res[0].top);
				this.maskHeight = res[0].top
			});
